...
proc Tree::_draw_node { path node x0 y0 deltax deltay padx showlines } {
    ...
    if {$win != "" } {
		set x [expr $x1+$padx+5]
	} else { 
		set x [expr $x1+$padx]
    }
	
    $path:cmd create text $x $y0 \
    ...
    if { [set win [Widget::getoption $path.$node -window]] != "" } {
        $path:cmd create image [expr $x1-10] $y0 -image $win -anchor w -tags "win i:$node"
    } 
        ...
        if { $flag & 48 } {
            ...
            if { [string length $win] } {
                if { ![string compare $type "win"] } {
                    ...
                    $path:cmd delete $idi
                    $path:cmd create image $x0 $y0 -image $win -anchor w -tags "win i:$node"
                }
            } 
