...
#    Copyright (C) 2000 Simon Edwards, Proprius Consulting Ltd                #
#                                                                             #
...
namespace export init end register_routine parse
...
proc register_routine {id path proc} {
variable xml_stack
variable xml_procs
	...
	lappend xml_procs($id) $path $proc
	return 0
}

proc xml_tag_attrs_to_str {attrstr} {
	...
	while {1} {
		...
		set a2 [xml_handle_bs_string $a2]
		lappend attrs $a1 $a2
		regsub {([[:alnum:]\-_]+)="([^"]*)"} $attrstr {} xx
		set attrstr $xx
	}

	if {[llength $attrstr] != 0} {
...
proc xml_handle_bs_string {str} {
	set c 0
	set x [string length $str]
	set nstr ""
	while { $c <= $x } {
		append nstr [xml_handle_bs c $str]
	}
 	return $nstr
}
...
variable xml_hadtoplevel
	...
	if { $have_default == -1 || [expr $have_default % 2] == 1 } {
		set defproc ""
	} else {
		incr have_default
	...
	while {! [eof $xml_file($id)]} {
		...
		while { $c < $ll } {
			...
			if { $status == 1 } {
				...
				if { "$ch" != ">" } {
					...
					if { "$tag" != "" && ("$ch" == " " || "$ch" == "\t") } {
						...
						if { [catch {set ctag_attrs [xml_tag_attrs_to_str $xx]}] } {
			...
			if { $status == 2 } {
				...
				append cdata [xml_handle_bs c $cline]
				continue
			}
			if { $status == 3 } {
				if { "$etag" == "" && "$ch" == "?" } {
				...
				if { $x ==-1 || [expr $x % 2] == 1 } {
					...
					if { "$defproc" != "" } {
						...
						set r [$defproc "$cstack" "$cdata" myarr2 "$xml_attrs_stack($cstack)" myarr]
						if { $r != "0" } {
